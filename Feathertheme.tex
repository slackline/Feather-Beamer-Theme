...
{ % is placed on the title page
      \textbf{Statistical Models with \href{https://patsy.readthedocs.io/en/latest/}{\texttt{Patsy}}}
}

\subtitle[Defining Statistical Models with \texttt{Patsy}]
%{
%      \textbf{v. 1.0.0}
%}
...
{      Neil Shephard \\
      {\ttfamily neil.shephard@thefloow.com}
}
...
%-------------------------------------------------------
  ...
  \begin{itemize}
    \item<1-> Statistical Modelling requires data.
    \item<2-> Typically one outcome/dependant/exogenous variable and multiple predictor/independant/endogenous variables.
    \item<3-> Data stored in Pandas \href{https://pandas.pydata.org/pandas-docs/stable/generated/pandas.DataFrame.html}{\texttt{pd.DataFrame()}}
    \item<4-> May have many variables not of immediate interest.
    \item<5-> How to pass data and information on statistical model to methods in packages like \href{http://scikit-learn.org/stable/}{\texttt{}scikit-learn}} and \href{http://www.statsmodels.org/stable/}{\texttt{statsmodels}}?
  \end{itemize}
\end{frame}
...
\begin{frame}{Handling Data}{Sample Data}
%-------------------------------------------------------
...
\begin{lstlisting}
import PyDataset
data = pd.DataFrame([{'outcome' : np.rand()},
                     {'pred1' : np.rand()},
                     {'pred2' : np.rand()},
                     {'pred3' : np.rand()},
                     {'pred4' : np.rand()},
